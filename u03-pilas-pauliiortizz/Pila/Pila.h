...
public:
    ...
    void reemplazar(T viejo, T nuevo);
};
...
/**
 * Imprime todos los valores almacenados en la pila
 * @tparam T
 */
template<class T>
...
/**
 * Reemplaza todas las ocurrencias de un valor viejo por un nuevo valor en la pila
 * @tparam T
 * @param viejo Valor que se desea reemplazar
 * @param nuevo Nuevo valor por el que se reemplazará
 */
template<class T>
void Pila<T>::reemplazar(T viejo, T nuevo) {
    Pila<T> pilaTemp;

    // Copiar todos los elementos a una pila temporal, reemplazando los valores viejos por los nuevos
    while (!esVacia()) {
        T valor = pop();
        if (valor == viejo) {
            valor = nuevo;
    ...
    // Copiar los elementos de la pila temporal de vuelta a la pila original
